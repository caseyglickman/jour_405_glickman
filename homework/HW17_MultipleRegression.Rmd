...
There seems to be a correlation between counties voter turnout and the percent of unemployment. Counties with the highest voter turnout have a lower percentage of people that are unemployed, and counties with the lowest voter turnout have a higher unemployment percentage. Between the independent variables there is also a correlation between income and the percent of people with a bachelors. Counties with the highest also have a higher percentage of people with a bachelors degree. Population as an independent variable does not seem to be correlated to voter turnout. Looking at the scatter plot, the percent of people unemployed has has the most clear linear relationship to voter turnout. The percent of the population that is white also seems to have a linear relationship with voter turnout. Population and income don't have a linear relationship. 
...
The median residual is .000494 which is very small and tells us that the difference between the actual and predicted values are small, therefore the model is centered around the actual data points. The model predicts a high negative correlation between unemployment and voter turnout with a coefficient of -1.768e+00. The model also predicts a positive correlation between percentage of white people and voter turnout which we can see because the coefficient is 1.041e-01. Overall, the variables that are the least significant seem to be population and income because both of these variables have very small coefficients. Overall I would say the model is useful. We can further see this because the adjusted R-squared value is 0.8023, which is close to 1 meaning that the model is a good fit for the data. The model is also statistically significant because the f statistic is large and the p-value is very small. 
...
Model 2 looks at unemployment and the percent of white people and their effect on voter turnout. This model has a higher  adjusted R-squared value compared to the other two models and is therefore more significant. Model 3 looks at the effect of bachelors degree and age - this model has the lowest adjusted r-squared meaning it is not as significant and the linear relationship between those variables and voter turnout is not as strong.
...
# Model 3
model3 <- lm(turnout ~  pct_bachelors_higher + median_age, data = md_turnout)
summary(model3)
...
I would use this analysis to discuss the effect of a counties unemployment rate on voter turnout. Unemployment rates have a strong negative correlation to voter turnout that is statically significant. In covering a state-wide election, like the governors race we could examine how counties with higher unemployment rates likely had lower voter turnout and what factors might be at play like a lack of civic engagement or a lack of time and resources that are effecting peoples ability to vote. We could talk to residents who are unemployed about why the chose to vote or not and what barriers the are facing that might prevent them from voting. 