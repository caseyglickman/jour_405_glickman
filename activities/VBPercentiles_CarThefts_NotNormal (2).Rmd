...
If you were to collect data about social media usage across all age groups, the data would likely not have a normal distribution and would be skewed towards younger people. Another example could be collecting data on college campuses about what percentage of students are commuters. If you were to collect that data at a state university like UMD, the data would likely be skewed towards computer students, versus if you were at a private university. 

As a reporter, when reporting on a story about data that isn't normally distributed, it is important to be explicit about where you collected the data and always mention the margin of error. It is also important to discuss outliers in the data and use median instead of mean in your statistical calculations to avoid the outliers skewing the data.
...
Maryland is in the 95th percentile along with Creighton and Merrimack which means that they are in the top 5% of volleyball performers. Maryland is performing better than 95% of teams and is perfmoing worse than just 5% of teams. They also have 223 total aces which puts them in the top 20 teams with the highest total aces.  
...
The data is skewed to the left because the mode is higher than the mean and the data has a tail extending to the left which means we know the data is skewed left 
