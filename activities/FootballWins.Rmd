...
This is a useful model because the p-value is very low which suggests that the model is statically significant. The model is useful in showing which factors actually have a significant impact on influencing the teams number of wins.
...
Points scored has the most impact on wins. In the visual model we can see that the points score has the largest circle which indicates that points scored has the biggest impact on wins. The model also shows that each additional point scored is associated with an increase in wins by about 0.039. Yards gained does not have an impact on wins. The yards gained coefficient is -0.001329 which suggests a very small negative impact of yards gained on wins, but this effect is not statistically significant.

## Using the Model

Without using the words "multiple regression model", describe how you could use this model to cover a team or game.

I would use this model to cover a team who has had high yards gained throughout their season but has not had a lot of wins. I could then use this model to explain that there is not a statistically significant correlation between yards gained and wins which explains this teams performance. 